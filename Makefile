...
JAVAC_FLAGS	 = -nowarn -Xlint:-removal -Xlint:-deprecation -Xlint:-removal -Xlint:none -g -classpath ".:./lib/js.jar:./lib/jzlib.jar"
JAVADOC		 = javadoc
JAR			 = jar
...
# MODIFIED: Dynamically discover all packages under 'com' instead of explicit list
PACKAGES := $(shell $(FIND) com -type d | $(SED) -e 's/^com\///' -e 's/\//\./g' | grep . | $(SED) -e 's/^/com./')

# MODIFIED: Use the dynamic PACKAGES
...
# MODIFIED: Use dynamic ALL_PACKAGES
