...
function decompressChunk(inflater, chunk) 
{
	inflater.push(chunk, pako.Z_SYNC_FLUSH);
	var strm = inflater.strm;
	if(strm.next_out > 0)
	{
		var pending = strm.output.subarray(0, strm.next_out);
		inflater.onData(pending);
		strm.next_out = 0;
		strm.avail_out = strm.output.length;
	}
	if((inflater.err) || (inflater.chunks.length === 0))
		return new Uint8Array();
	var totalLength = inflater.chunks.reduce(function(acc, ch) { return acc + ch.length; }, 0);
	var result = new Uint8Array(totalLength);
	var offset = 0;
	for(var ch of inflater.chunks) 
	{
