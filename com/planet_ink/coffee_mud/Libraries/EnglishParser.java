...
public class EnglishParser extends StdLibrary implements EnglishParsing
{
    ...
    public Vector preCommandParser(Vector CMDS)
    {
    	...
    	DVector parser=Scripts.getLanguageParser("COMMAND-PRE-PROCESSOR");
    ...
    {
    	DVector parser=Scripts.getLanguageParser("ITEM-PRE-PROCESSOR");
    	if((parser==null)||(item==null)) return null;
    	Pattern pattern=null;
    	Matcher matcher=null;
    	String oldStr=item;
		for(int p=0;p<parser.size();p++)
		{
			if(((String)parser.elementAt(p,1)).equals("REPLACE"))
			{
	    		...
	    		if(matcher.find())
	            	for(int i=0;i<=matcher.groupCount();i++)
	            		item=CMStrings.replaceAll(item,"\\"+i,matcher.group(i));
			}
		}
    	return item.length()>=oldStr.length()?null:item;
    }
    public String failedItemParser(String item)
    {
    	DVector parser=Scripts.getLanguageParser("ITEM-FAIL-PROCESSOR");
    	if((parser==null)||(item==null)) return null;
    	Pattern pattern=null;
    	Matcher matcher=null;
    	String oldStr=item;
