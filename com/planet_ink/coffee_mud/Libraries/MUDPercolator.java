...
public class MUDPercolator extends StdLibrary implements AreaGenerationLibrary
{
	...
	protected Room layOutRooms(final Area A, final LayoutManager layoutManager, final int size, final int direction, final XMLTag piece, final Map<String,Object> defined) throws CMException
	{
		...
		final int preNum = A.properSize();
		final Room magicRoom = processRoom(A,direction,piece,magicRoomNode,groupDefined);
		if(!magicRoom.roomID().endsWith("#"+preNum))
			Log.errOut("MudPercolator","Magic room id "+magicRoom.roomID()+" is not #"+preNum+", LM="+layoutManager.name());
		if(produceExternalExits)
			mostRooms.put(Integer.valueOf(opDirection), magicRoomNode);
			...
			if(CMath.s_bool(valPiece.getParmValue("LLM")))
			{
				try
				{
					if(!defined.containsKey("SYSTEM_LLM_Object"))
					{
						...
						defined.put("SYSTEM_LLM_Object", CMLib.protocol().createLLMSession(categories, null,null));
					}
