...
public final class IMC2Driver extends Thread
{
	...
	final public List<CMChannel> rebuildChannelMap()
	{
		...
		for(final Enumeration<String> e=chan_conf.keys();e.hasMoreElements();)
		{
			...
			final CMChannel chan=CMLib.channels().createNewChannel(name, imudChans, mask, new HashSet<ChannelFlag>(),"","");
			map.add(chan);
		}
		return map;
	}
...
//
	...
	final void imc_recv_is_alive(final imc_char_data d, final String from, final String path,
						   final String version, final String netname, final String url,
	...
	final public void exec_commands(final PACKET p)
	{
		...
		if (p.type.equals("is-alive"))
		{
			...
			imc_recv_is_alive(d, p.i.from, p.i.path,
							  ...
							  imc_getkey(p, "url", "Unknown"),
							  imc_getkey(p, "host", "Unknown"),
							  imc_getkey(p, "port", "Unkown"));
		}
