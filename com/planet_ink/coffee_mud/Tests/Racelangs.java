...
*/
public class Racelangs extends StdTest
{
	@Override
	public String ID()
	{
	...
	public String doTest(final MOB mob, final int metaFlags, final String what, final List<String> commands)
	{
		for(final Enumeration<Race> r=CMClass.races();r.hasMoreElements();)
		{
			...
			if(RA!=null)
			{
				...
				for(final Quad<String,Integer,Integer,Boolean> Q : RA.culturalAbilities())
				{
					final Ability A=CMClass.getAbilityPrototype(Q.first);
					if(A instanceof Language)
						buf.append("C:").append(A.ID()).append(" ");
				}
				for(final Ability A : RA.racialAbilities(null))
				{
					if(A instanceof Language)
						buf.append("R:").append(A.ID()).append(" ");
				}
				for(final Ability A : RA.racialEffects(null))
				{
					if(A instanceof Language)
