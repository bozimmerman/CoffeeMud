...
public class Buy extends StdCommand
{
		...
		throws java.io.IOException
	{
		MOB mobFor=null;
		if((commands.size()>2)
		&&(((String)commands.elementAt(commands.size()-2)).equalsIgnoreCase(getScr("Buy","for"))))
		{
		...
		&&(CMath.s_int((String)commands.firstElement())>0))
		{
			...
			commands.setElementAt(getScr("Buy","all"),0);
		}

		String whatName=CMParms.combine(commands,0);
		Vector V=new Vector();
		boolean allFlag=((String)commands.elementAt(0)).equalsIgnoreCase(getScr("Buy","all"));
		if(whatName.toUpperCase().startsWith(getScr("Buy","alldot"))){ allFlag=true; whatName=getScr("Buy","allup")+whatName.substring(4);}
		if(whatName.toUpperCase().endsWith(getScr("Buy","dotall"))){ allFlag=true; whatName=getScr("Buy","allup")+whatName.substring(0,whatName.length()-4);}
		int addendum=1;
		do
		{
