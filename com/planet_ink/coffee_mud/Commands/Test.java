            ...
            if(what.equalsIgnoreCase("abilityobjectcache"))
            {
                ...
                while(H.size()>0)
                {
                    ...
                    for(Iterator i=H.keySet().iterator();i.hasNext();)
                    {
                        E=i.next();
                        I=(Integer)H.get(E);
                        if(I.intValue()>=max){ max=I.intValue(); W=E;}
                    }
                    H.remove(W);
                    if(W instanceof Environmental)
                        V.addElement(((Environmental)W).ID()+": "+((Environmental)W).name()+": "+max);
                    else
                        V.addElement(W.getClass().getName()+": "+max);
                }
                for(int i=0;i<20;i++)
                    mob.tell(((String)V.elementAt(i)));
                V=new Vector(NULLS.size());
                while(NULLS.size()>0)
                {
                    int max=Integer.MIN_VALUE;
                    Object W=null;
                    for(Iterator i=NULLS.keySet().iterator();i.hasNext();)
                    {
                        E=i.next();
                        I=(Integer)NULLS.get(E);
                        if(I.intValue()>=max){ max=I.intValue(); W=E;}
                    }
                    NULLS.remove(W);
                    V.addElement(((String)W)+": "+max);
                }
                for(int i=0;i<20;i++)
                    mob.tell(((String)V.elementAt(i)));
            }
