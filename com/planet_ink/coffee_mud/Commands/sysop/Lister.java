...
public class Lister
{
	...
	private static void fillCheckDeviations(Room R, String type, Vector check)
	{
	...
	private static String getDeviation(int val, Hashtable vals, String key)
	{
	...
	private static String getDeviation(int val, int val2)
	{
	...
	private static int getRoomDirection(Room R, Room toRoom, Vector ignore)
	{
		for(int d=0;d<Directions.NUM_DIRECTIONS;d++)
	...
	public static String trailTo(Room R1, Vector commands)
	{
		...
		SaucerSupport.getRadiantRooms(R1,set,false,false,true,null,Integer.MAX_VALUE);
	...
	public static String trailTo(Room R1, Vector set, String where, boolean confirm)
	{
		...
		if(set.size()==0)
			SaucerSupport.getRadiantRooms(R1,set,false,false,true,R2,Integer.MAX_VALUE);
		int foundAt=-1;
		for(int i=0;i<set.size();i++)
		{
			Room R=(Room)set.elementAt(i);
	...
	public static StringBuffer deviations(MOB mob, String rest)
	{
		...
		for(int c=0;c<check.size();c++)
		{
			if(check.elementAt(c) instanceof Item)
			{
				...
				if(I instanceof Weapon)
					W=(Weapon)I;
				Hashtable vals=SaucerSupport.timsItemAdjustments(
	...
	public static String listEnvResources()
	{
		StringBuffer str=new StringBuffer("");
	...
	public static void list(MOB mob, Vector commands)
	{
		...
		if("COMMON".startsWith(listThis))
			s.rawPrintln(reallyList(CMClass.abilities(),Ability.COMMON_SKILL).toString());
		else
		...
		if("UNLINKEDEXITS".startsWith(listWord))
			...
			s.rawPrintln("Can't list those, try ITEMS, POISONS, COMMON, DISEASES, ARMOR, WEAPONS, MOBS, ROOMS, LOCALES, EXITS, RACES, CLASSES, MAGIC, SPELLS, SONGS, PRAYERS, BEHAVIORS, SKILLS, THIEFSKILLS, PROPERTIES, TICKS, LOG, USERS, SESSIONS, THREADS, BUGS, IDEAS, TYPOS, REPORTS, BANNED, WORDS, RESOURCES, ENVRESOURCES, SUBOPS, TRAILTO, or AREA.");
	}
}