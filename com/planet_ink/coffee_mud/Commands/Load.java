...
public class Load extends StdCommand
{
	...
	public final String[] combine(final String[] set1, final String[] set2)
	{
		final String[] fset=new String[set1.length+set2.length];
		for(int x=0;x<set1.length;x++)
			fset[x]=set1[x];
		for(int x=0;x<set2.length;x++)
			fset[set1.length+x]=set2[x];
		return fset;
	}
	
	public final String ARCHON_LIST[]=combine(new String[]{"RESOURCE","FACTION"},CMClass.OBJECT_DESCS);
