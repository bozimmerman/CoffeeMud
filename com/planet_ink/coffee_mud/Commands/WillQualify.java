...
public class WillQualify extends BaseAbleLister{
        ...
        private String[] access={"WILLQUALIFY","WILL"};
...
System.out.println("!");				
                ...
                for (int l = 0; l <= highestLevel; l++) {
                        ...
                        for (Enumeration a = CMAble.getClassAbles(Class); a.hasMoreElements(); ) {
                                ...
                                Log.sysOut("WillQual",Class+"/"+maxLevel+": "+A.ID()+"("+(CMAble.getQualifyingLevel(Class,false,A.ID()))+")");
                                if ((CMAble.getQualifyingLevel(Class,false,A.ID()) <= highestLevel)
                                    && (CMAble.getQualifyingLevel(Class,false,A.ID()) > -1)
        ...
        public boolean execute(MOB mob, Vector commands)
                        throws java.io.IOException
                {
                        ...
                        if (level > 0) {
                                ...
                                msg.append(getQualifiedAbilities(mob,className,level,""));
                                if(!mob.isMonster())
                                {
