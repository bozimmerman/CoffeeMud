...
public class StdMOB implements MOB
{
    ...
    //protected void finalize() { CMClass.unbumpCounter(this,CMClass.CMObjectType.MOB); }//removed for mem & perf
    ...
    public CMObject copyOf()
    {
        try
        {
            ...
            //CMClass.bumpCounter(E,CMClass.CMObjectType.MOB);//removed for mem & perf
    ...
    public void recoverPhyStats()
    {
        ...
        final Rideable riding=riding(); if(riding!=null) riding.affectPhyStats(this,phyStats);
        final Deity deity=getMyDeity(); if(deity!=null) deity.affectPhyStats(this,phyStats);
    ...
    public void recoverMaxState()
    {
        ...
        final Rideable riding=riding(); if(riding!=null) riding.affectCharState(this,maxState);
        final int num=charStats.numClasses();
        for(int c=0;c<num;c++)
    ...
    {
        final String mxp="^<!ENTITY vicmaxhp \""+maxState().getHitPoints()+"\"^>^<!ENTITY vichp \""+curState().getHitPoints()+"\"^>^<Health^>^<HealthText \""+CMStrings.removeColors(name())+"\"^>";
        if((charStats()!=null)&&(charStats().getMyRace()!=null))
        ...
        if((msg.sourceCode()!=CMMsg.NO_EFFECT)&&(msg.amISource(this)))
        {
            ...
            if(!msg.sourceMajor(CMMsg.MASK_ALWAYS))
            {
                ...
                case CMMsg.TYP_CLOSE:
                    ...
                    {
                        if((msg.target()!=null)
                        &&((msg.target() instanceof Exit)||(srcM.isMine(msg.target()))))
                            break;
                        tell("Not while you are fighting!");
                        return false;
                    }
        ...
        if((msg.targetMinor()!=CMMsg.NO_EFFECT)&&(msg.amITarget(this)))
        {
            ...
            if(CMath.bset(msg.targetMajor(),CMMsg.MASK_MALICIOUS))
            {
                   ...
                   ||((srcM==this)&&(srcM.isMonster()))))
                {
                    ...
                    srcM.setVictim(null);
                    if(victim==srcM) setVictim(null);
                    return false;
                }

                if((!isMonster())&&(!srcM.isMonster())
                &&(soulMate()==null)
                &&(srcM.soulMate()==null)
                &&(!CMSecurity.isAllowed(this,location(),"PKILL"))&&(!CMSecurity.isAllowed(srcM,srcM.location(),"PKILL"))
    ...
    {
        final Deity deity=getMyDeity();
        if(deity!=null) deity.executeMsg(this,msg);
    ...
    public Ability findAbility(String ID)
    {
        ...
        if(A==null) A=(Ability)CMLib.english().fetchEnvironmental(abilitys,ID,false);
