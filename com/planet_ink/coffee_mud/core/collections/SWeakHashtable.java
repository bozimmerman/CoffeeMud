...
/**
 * Weak version of the SHashtable backed by WeakHashMap
 * 
 * @author Bo
 *
 * @param <K> the key
 * @param <F> the value
 */
public class SWeakHashtable<K, F> implements CMap<K, F>, java.io.Serializable
{
	...
	public SWeakHashtable()
	{
		super();
		H = new WeakHashMap<K, F>();
	}

	public SWeakHashtable(final int size)
	{
		super();
		this.H = new WeakHashMap<K, F>(size);
	}

	public SWeakHashtable(final Enumeration<Pair<K, F>> e)
	{
		super();
		this.H = new WeakHashMap<K, F>();
		if (H != null)
		{
	...
	public SWeakHashtable(final Map<K, F> H)
	{
		super();
		if(H instanceof SWeakHashtable)
			this.H = new WeakHashMap<K,F>(((SWeakHashtable<K,F>)H).H);
		else
		{
			this.H = new WeakHashMap<K, F>();
			if (H != null)
	...
	public SWeakHashtable(final Object[][] H)
	{
		super();
		this.H = new WeakHashMap<K, F>();
		if (H != null)
	...
	@SuppressWarnings("unchecked")
	public SWeakHashtable(final SWeakHashtable<K, F> H, final boolean reverse)
	{
		super();
		this.H = new WeakHashMap<K, F>();
		if (H != null)
	...
	public SWeakHashtable(final SWeakHashtable<K, F> H)
	{
		this(H,false);
	}
	...
	public SWeakHashtable<K, F> copyOf()
	{
		return new SWeakHashtable<K, F>(H);
	}
	...
	public F put(final K arg0, final F arg1)
	{
		final ReentrantLock lock = this.lock;
		lock.lock();
		final WeakHashMap<K, F> H2 = new WeakHashMap<K, F>(H);
		try
		{
			dirty=true;
