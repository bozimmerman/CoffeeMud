...
public class MOBloader {
	...
	public boolean DBReadUserOnly(MOB mob) {
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHAR WHERE CMUSERID='"
					+ mob.Name() + "'");
			while (R.next()) {
				CharStats stats = mob.baseCharStats();
				CharState state = mob.baseState();
				PlayerStats pstats = (PlayerStats) CMClass
						.getCommon("DefaultPlayerStats");
				...
				stats.setStat(CharStats.STAT_STRENGTH, CMath
						.s_int(DBConnections.getRes(R, "CMSTRE")));
				stats.setMyRace(CMClass.getRace(DBConnections.getRes(R,
						"CMRACE")));
				stats.setStat(CharStats.STAT_DEXTERITY, CMath
						.s_int(DBConnections.getRes(R, "CMDEXT")));
				stats.setStat(CharStats.STAT_CONSTITUTION, CMath
						.s_int(DBConnections.getRes(R, "CMCONS")));
				stats.setStat(CharStats.STAT_GENDER, DBConnections.getRes(R,
						"CMGEND").charAt(0));
				stats.setStat(CharStats.STAT_WISDOM, CMath.s_int(DBConnections
						.getRes(R, "CMWISD")));
				stats.setStat(CharStats.STAT_INTELLIGENCE, CMath
						.s_int(DBConnections.getRes(R, "CMINTE")));
				stats.setStat(CharStats.STAT_CHARISMA, CMath
						.s_int(DBConnections.getRes(R, "CMCHAR")));
				state.setHitPoints(CMath.s_int(DBConnections
						.getRes(R, "CMHITP")));
				stats.setMyLevels(DBConnections.getRes(R, "CMLEVL"));
				int level = 0;
				for (int i = 0; i < mob.baseCharStats().numClasses(); i++)
					level += stats.getClassLevel(mob.baseCharStats()
							.getMyClass(i));
				mob.baseEnvStats().setLevel(level);
				state.setMana(CMath.s_int(DBConnections.getRes(R, "CMMANA")));
				state.setMovement(CMath
						.s_int(DBConnections.getRes(R, "CMMOVE")));
				...
				mob.setExperience(CMath
						.s_int(DBConnections.getRes(R, "CMEXPE")));
				mob.setExpNextLevel(CMath.s_int(DBConnections.getRes(R,
						"CMEXLV")));
				mob.setWorshipCharID(DBConnections.getRes(R, "CMWORS"));
				mob
						.setPractices(CMath.s_int(DBConnections.getRes(R,
								"CMPRAC")));
				mob.setTrains(CMath.s_int(DBConnections.getRes(R, "CMTRAI")));
				mob
						.setAgeHours(CMath.s_long(DBConnections.getRes(R,
								"CMAGEH")));
				mob.setMoney(CMath.s_int(DBConnections.getRes(R, "CMGOLD")));
				mob.setWimpHitPoint(CMath.s_int(DBConnections.getRes(R,
						"CMWIMP")));
				mob.setQuestPoint(CMath
						.s_int(DBConnections.getRes(R, "CMQUES")));
				String roomID = DBConnections.getRes(R, "CMROID");
				if (roomID == null)
					roomID = "";
				int x = roomID.indexOf("||");
				if (x >= 0) {
					mob.setLocation(CMLib.map()
							.getRoom(roomID.substring(x + 2)));
					roomID = roomID.substring(0, x);
				}
				mob.setStartRoom(CMLib.map().getRoom(roomID));
				pstats.setLastDateTime(CMath.s_long(DBConnections.getRes(R,
						"CMDATE")));
				pstats.setChannelMask((int) DBConnections.getLongRes(R,
						"CMCHAN"));
				mob.baseEnvStats().setAttackAdjustment(
						CMath.s_int(DBConnections.getRes(R, "CMATTA")));
				mob.baseEnvStats().setArmor(
						CMath.s_int(DBConnections.getRes(R, "CMAMOR")));
				mob.baseEnvStats().setDamage(
						CMath.s_int(DBConnections.getRes(R, "CMDAMG")));
				mob.setBitmap(CMath.s_int(DBConnections.getRes(R, "CMBTMP")));
				mob.setLiegeID(DBConnections.getRes(R, "CMLEIG"));
				mob.baseEnvStats().setHeight(
						(int) DBConnections.getLongRes(R, "CMHEIT"));
				mob.baseEnvStats().setWeight(
						(int) DBConnections.getLongRes(R, "CMWEIT"));
				pstats.setPrompt(DBConnections.getRes(R, "CMPRPT"));
				String colorStr = DBConnections.getRes(R, "CMCOLR");
				if ((colorStr != null) && (colorStr.length() > 0)
						&& (!colorStr.equalsIgnoreCase("NULL")))
				...
				Vector V9 = CMParms.parseSemicolons(CMLib.xml().returnXMLValue(
						buf, "TATTS"), true);
				...
				V9 = CMParms.parseSemicolons(CMLib.xml().returnXMLValue(buf,
						"EDUS"), true);
				...
				if (pstats.getBirthday() == null)
					stats.setStat(CharStats.STAT_AGE, pstats
							.initializeBirthday((int) Math.round(CMath.div(mob
									.getAgeHours(), 60.0)), stats.getMyRace()));
				mob.setImage(CMLib.xml().returnXMLValue(buf, "IMG"));
				Vector CleanXML = CMLib.xml().parseAllXML(
						DBConnections.getRes(R, "CMMXML"));
	...
	public void DBRead(MOB mob) {
		...
		mob.baseEnvStats().setDisposition(
				EnvStats.IS_NOT_SEEN | EnvStats.IS_SNEAKING);
		mob.envStats().setDisposition(
				EnvStats.IS_NOT_SEEN | EnvStats.IS_SNEAKING);
		DBReadUserOnly(mob);
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHIT WHERE CMUSERID='"
					+ mob.Name() + "'");
			...
			while (R.next()) {
				...
				if (newItem == null)
					...
					newItem.setMiscText(DBConnections
							.getResQuietly(R, "CMITTX"));
					...
					newItem.setUsesRemaining((int) DBConnections.getLongRes(R,
							"CMITUR"));
					newItem.baseEnvStats().setLevel(
							(int) DBConnections.getLongRes(R, "CMITLV"));
					newItem.baseEnvStats().setAbility(
							(int) DBConnections.getLongRes(R, "CMITAB"));
					newItem.baseEnvStats().setHeight(
							(int) DBConnections.getLongRes(R, "CMHEIT"));
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHAB WHERE CMUSERID='"
					+ mob.Name() + "'");
			while (R.next()) {
				...
				if (proficiency == Integer.MIN_VALUE) {
					...
					if (newBehavior == null)
						Log.errOut("MOB", "Couldn't find behavior '"
								+ abilityID + "'");
					else {
					...
					if (newAbility == null)
						...
						if ((proficiency < 0)
							...
							if (proficiency == Integer.MAX_VALUE) {
								...
								newAbility.setMiscText(DBConnections.getRes(R,
										"CMABTX"));
							} else {
								...
								Ability newAbility2 = (Ability) newAbility
										.copyOf();
								mob.addNonUninvokableEffect(newAbility);
		...
		if (mob.baseCharStats() != null) {
			mob.baseCharStats().getCurrentClass().startCharacter(mob, false,
					true);
			...
			if (oldHeight > 0)
				mob.baseEnvStats().setHeight(oldHeight);
		}
	...
	public void vassals(MOB mob, String liegeID) {
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHAR WHERE CMLEIG='"
					+ liegeID + "'");
			...
			if (R != null)
				while (R.next()) {
					...
					if (M == null) {
						...
						int x = cclass.lastIndexOf(";");
						if ((x > 0) && (x < cclass.length() - 2))
							cclass = CMClass.getCharClass(
									cclass.substring(x + 1)).name();
						String race = (CMClass.getRace(DBConnections.getRes(R,
								"CMRACE"))).name();
						String lvl = DBConnections.getRes(R, "CMLEVL");
						x = lvl.indexOf(";");
						int level = 0;
						...
						int exp = CMath
								.s_int(DBConnections.getRes(R, "CMEXPE"));
						int exlv = CMath.s_int(DBConnections
								.getRes(R, "CMEXLV"));
						head.append("[");
						head.append(CMStrings.padRight(race, 8) + " ");
						head.append(CMStrings.padRight(cclass, 10) + " ");
						head.append(CMStrings.padRight(Integer.toString(level),
								4)
								+ " ");
			...
			for (Enumeration e = CMLib.map().players(); e.hasMoreElements();) {
				...
				if ((M.getLiegeID().equals(liegeID))
					...
					head.append("[");
					head.append(CMStrings.padRight(M.charStats().getMyRace()
							.name(), 8)
							+ " ");
					head.append(CMStrings.padRight(M.charStats()
							.getCurrentClass().name(
									M.charStats().getCurrentClassLevel()), 10)
							+ " ");
					head.append(CMStrings
							.padRight("" + M.envStats().level(), 4)
							+ " ");
					head.append(CMStrings.padRight(M.getExperience() + "/"
							+ M.getExpNextLevel(), 17));
	...
	public void DBReadFollowers(MOB mob, boolean bringToLife) {
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHFO WHERE CMUSERID='"
					+ mob.Name() + "'");
			while (R.next()) {
				...
				if (newMOB == null)
					...
					Room room = (location == null) ? newMOB.getStartRoom()
							: location;
					newMOB.setStartRoom(room);
					newMOB.setLocation(room);
					newMOB
							.setMiscText(DBConnections.getResQuietly(R,
									"CMFOTX"));
					newMOB.baseEnvStats().setLevel(
							((int) DBConnections.getLongRes(R, "CMFOLV")));
					newMOB.baseEnvStats().setAbility(
							(int) DBConnections.getLongRes(R, "CMFOAB"));
					...
					if ((newMOB.getStartRoom() != null)
							&& (CMLib.utensils().doesHavePriviledgesHere(mob,
									newMOB.getStartRoom()))
							&& ((newMOB.location() == null) || (!CMLib
									.utensils().doesHavePriviledgesHere(mob,
											newMOB.location()))))
						newMOB.setLocation(newMOB.getStartRoom());
					if (bringToLife) {
						newMOB.bringToLife(mob.location(), true);
						mob.location().showOthers(newMOB, null,
								CMMsg.MSG_OK_ACTION, "<S-NAME> appears!");
					}
				}
			}
		} catch (Exception sqle) {
	...
	public void DBUpdateEmail(MOB mob) {
		...
		DB.update("UPDATE CMCHAR SET" + "  CMEMAL='" + pstats.getEmail() + "'"
		...
		try {
			...
			if (R != null)
				while (R.next()) {
					...
					long lastDateTime = CMath.s_long(DBConnections.getRes(R,
							"CMDATE"));
	...
	public void DBUpdateClan(String name, String clan, int role) {
		...
		DB.update("UPDATE CMCHAR SET" + "  CMCLAN='" + clan + "',"
				+ "  CMCLRO=" + role + "" + "  WHERE CMUSERID='" + name + "'");
	}
	...
	public void DBUpdateJustMOB(MOB mob) {
		...
		String strStartRoomID = (mob.getStartRoom() != null) ? CMLib.map()
				.getExtendedRoomID(mob.getStartRoom()) : "";
		String strOtherRoomID = (mob.location() != null) ? CMLib.map()
				.getExtendedRoomID(mob.location()) : "";
		...
		DB.update("UPDATE CMCHAR SET" + "  CMPASS='" + pstats.password() + "'"
				...
				+ ", CMCONS="
				+ mob.baseCharStats().getStat(CharStats.STAT_CONSTITUTION)
				+ ", CMGEND='"
				+ ((char) mob.baseCharStats().getStat(CharStats.STAT_GENDER))
				+ "'" + ", CMWISD="
				+ mob.baseCharStats().getStat(CharStats.STAT_WISDOM)
				+ ", CMINTE="
				+ mob.baseCharStats().getStat(CharStats.STAT_INTELLIGENCE)
				+ ", CMCHAR="
				+ mob.baseCharStats().getStat(CharStats.STAT_CHARISMA)
				+ ", CMHITP=" + mob.baseState().getHitPoints() + ", CMLEVL='"
				+ mob.baseCharStats().getMyLevelsStr() + "'" + ", CMMANA="
				+ mob.baseState().getMana() + ", CMMOVE="
				+ mob.baseState().getMovement() + ", CMALIG=" + "-1"
				+ ", CMEXPE=" + mob.getExperience() + ", CMEXLV="
				+ mob.getExpNextLevel() + ", CMWORS='" + mob.getWorshipCharID()
				+ "'" + ", CMPRAC=" + mob.getPractices() + ", CMTRAI="
				+ mob.getTrains() + ", CMAGEH=" + mob.getAgeHours()
				+ ", CMGOLD=" + mob.getMoney() + ", CMWIMP="
				+ mob.getWimpHitPoint() + ", CMQUES=" + mob.getQuestPoint()
				+ ", CMROID='" + strStartRoomID + "||" + strOtherRoomID + "'"
				+ ", CMDATE='" + pstats.lastDateTime() + "'" + ", CMCHAN="
				+ pstats.getChannelMask() + ", CMATTA="
				+ mob.baseEnvStats().attackAdjustment() + ", CMAMOR="
				+ mob.baseEnvStats().armor() + ", CMDAMG="
				+ mob.baseEnvStats().damage() + ", CMBTMP=" + mob.getBitmap()
				+ ", CMLEIG='" + mob.getLiegeID() + "'" + ", CMHEIT="
				+ mob.baseEnvStats().height() + ", CMWEIT="
				+ mob.baseEnvStats().weight() + ", CMPRPT='"
				+ pstats.getPrompt() + "'" + ", CMCOLR='"
				+ pstats.getColorStr() + "'" + ", CMCLAN='" + mob.getClanID()
				+ "'" + ", CMLSIP='" + pstats.lastIP() + "'" + ", CMCLRO="
				+ mob.getClanRole() + ", CMEMAL='" + pstats.getEmail() + "'"
				+ ", CMPFIL='" + pfxml.toString() + "'" + ", CMSAVE='"
				+ mob.baseCharStats().getSavesAsString() + "'" + ", CMMXML='"
				+ cleanXML.toString() + "'" + "  WHERE CMUSERID='" + mob.Name()
				+ "'");

		DB.update("UPDATE CMCHAR SET" + " CMDESC='" + mob.description() + "'"
				+ " WHERE CMUSERID='" + mob.Name() + "'");
	}
		...
		for (int i = 0; i < mob.inventorySize(); i++) {
			...
			if ((thisItem != null) && (!done.contains("" + thisItem))
				...
				String str = "INSERT INTO CMCHIT ("
						...
						+ " ',"
						+ "'"
						+ ((thisItem.container() != null) ? ("" + thisItem
								.container()) : "") + "',"
	...
	public void DBUpdateItems(MOB mob) {
		...
		if (DB.queryRows("SELECT * FROM CMCHIT  WHERE CMUSERID='" + mob.Name()
				+ "'") > 0)
	...
	public void DBUpdateFollowers(MOB mob) {
		...
		if (DB.queryRows("SELECT * FROM CMCHFO  WHERE CMUSERID='" + mob.Name()
				+ "'") > 0)
			Log
					.errOut("Failed to update followers for mob " + mob.Name()
							+ ".");
		Vector V = new Vector();
		for (int f = 0; f < mob.numFollowers(); f++) {
			...
			if ((thisMOB != null) && (thisMOB.isMonster())
				...
				String str = "INSERT INTO CMCHFO (" + "CMUSERID, " + "CMFONM, "
						+ "CMFOID, " + "CMFOTX, " + "CMFOLV, " + "CMFOAB"
						+ ") values (" + "'" + mob.Name() + "'," + f + ","
	...
	public void DBDelete(MOB mob) {
		if (mob.Name().length() == 0)
			return;
		Vector channels = CMLib.channels().getFlaggedChannelNames(
				"PLAYERPURGES");
		for (int i = 0; i < channels.size(); i++)
			CMLib.commands().postChannel((String) channels.elementAt(i),
					mob.getClanID(), mob.Name() + " has just been deleted.",
					true);
	...
	public void DBUpdateAbilities(MOB mob) {
		...
		if (DB.queryRows("SELECT * FROM CMCHAB  WHERE CMUSERID='" + mob.Name()
				+ "'") > 0)
			Log
					.errOut("Failed to update abilities for mob " + mob.Name()
							+ ".");
		Vector V = new Vector();
		...
		for (int a = 0; a < mob.numLearnedAbilities(); a++) {
			...
			if ((thisAbility != null) && (thisAbility.savable())) {
				...
				if (effectA != null) {
					if ((effectA.savable()) && (!effectA.canBeUninvoked())
							&& (!effectA.isAutoInvoked()))
						proficiency = proficiency - 200;
				}
				H.add(thisAbility.ID());

				String str = "INSERT INTO CMCHAB (" + "CMUSERID, " + "CMABID, "
						+ "CMABPF," + "CMABTX" + ") values (" + "'"
						+ mob.Name() + "'," + "'" + thisAbility.ID() + "',"
						+ proficiency + ",'" + thisAbility.text() + "'" + ")";
				V.addElement(str);
			}
		}
		for (int a = 0; a < mob.numEffects(); a++) {
			Ability thisAffect = mob.fetchEffect(a);
			if ((thisAffect != null) && (!H.contains(thisAffect.ID()))
					&& (thisAffect.savable()) && (!thisAffect.canBeUninvoked())) {
				String str = "INSERT INTO CMCHAB (" + "CMUSERID, " + "CMABID, "
						+ "CMABPF," + "CMABTX" + ") values (" + "'"
						+ mob.Name() + "'," + "'" + thisAffect.ID() + "',"
						+ Integer.MAX_VALUE + ",'" + thisAffect.text() + "'"
						+ ")";
				V.addElement(str);
			}
		}
		for (int b = 0; b < mob.numBehaviors(); b++) {
			...
			if ((thisBehavior != null) && (thisBehavior.isSavable())) {
				String str = "INSERT INTO CMCHAB (" + "CMUSERID, " + "CMABID, "
						+ "CMABPF," + "CMABTX" + ") values (" + "'"
						+ mob.Name() + "'," + "'" + thisBehavior.ID() + "',"
						+ Integer.MIN_VALUE + ",'" + thisBehavior.getParms()
						+ "'" + ")";
				V.addElement(str);
			}
		}
	...
	public void DBCreateCharacter(MOB mob) {
		...
		DB.update("INSERT INTO CMCHAR (" + "CMUSERID, " + "CMPASS, "
				+ "CMCLAS, " + "CMRACE, " + "CMGEND " + ") VALUES ('"
				+ mob.Name() + "','" + pstats.password() + "','"
	...
	public boolean DBUserSearch(MOB mob, String Login) {
		...
		if (mob != null) {
			if (mob.playerStats() == null)
				mob.setPlayerStats((PlayerStats) CMClass
						.getCommon("DefaultPlayerStats"));
			mob.setName("");
				...
				while (R.next()) {
					...
					if (Login.equalsIgnoreCase(username)) {
						...
						if (mob != null) {
							...
							// Acct Exp Code
							String buf = DBConnections.getRes(R, "CMPFIL");
							if (CMLib.xml().returnXMLValue(buf, "ACCTEXP")
									.length() > 0)
								mob.playerStats().setAccountExpiration(
										CMath
												.s_long(CMLib.xml()
														.returnXMLValue(buf,
																"ACCTEXP")));
	...
	public String[] DBFetchEmailData(String name) {
		...
		for (Enumeration e = CMLib.map().players(); e.hasMoreElements();) {
			...
			if ((M.Name().equalsIgnoreCase(name)) && (M.playerStats() != null)) {
				...
				data[1] = ""
						+ ((M.getBitmap() & MOB.ATT_AUTOFORWARD) == MOB.ATT_AUTOFORWARD);
				return data;
			}
		}
		DBConnection D = null;
		try {
			...
			if (R != null)
				while (R.next()) {
					...
					data[0] = temail;
					data[1] = ""
							+ ((btmp & MOB.ATT_AUTOFORWARD) == MOB.ATT_AUTOFORWARD);
					return data;
				}
		} catch (Exception sqle) {
			Log.errOut("MOB", sqle);
	...
	public String DBEmailSearch(String email) {
		...
		for (Enumeration e = CMLib.map().players(); e.hasMoreElements();) {
			...
			if ((M.playerStats() != null)
					&& (M.playerStats().getEmail().equalsIgnoreCase(email)))
				return M.Name();
		}
		try {
			D = DB.DBFetch();
