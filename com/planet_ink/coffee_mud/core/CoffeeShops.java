...
public class CoffeeShops implements Cloneable
{
                                                    ...
                                                    boolean includeSalesTax)
    {
        ...
        if(buyer==null) 
        {
            if(val.absoluteGoldPrice>0.0)
                val.absoluteGoldPrice=BeanCounter.abbreviatedRePrice(seller,val.absoluteGoldPrice);
            return val;
        }
                                                ...
                                                boolean buyNotView)
    {
        ...
        &&(shop.doIHaveThisInStock("$"+product.Name()+"$",buyer)))
        {
            if(buyNotView)
            {
                ...
                &&(price.absoluteGoldPrice>BeanCounter.getTotalAbsoluteShopKeepersValue(buyer,seller)))
                {
            ...
            if(product instanceof MOB)
            {
                ...
                {
                    if(seller.envStats().level() > (buyer.envStats().level() + CommonStrings.getIntVar(CommonStrings.SYSTEMI_FOLLOWLEVELDIFF)))
                    {
        ...
        if(inventory.size()>0)
        {
            ...
            for(int i=0;i<inventory.size();i++)
            {
                ...
                if(shownInInventory(E,buyer))
                {
                    ...
                    {
                        showPrice=BeanCounter.abbreviatedPrice(seller,price.absoluteGoldPrice);
            ...
            for(int i=0;i<inventory.size();i++)
            {
                ...
                if(shownInInventory(E,buyer))
                {
                    ...
                    if(price.experiencePrice>0)
                        ...
                        col=Util.padRight("["+BeanCounter.abbreviatedPrice(seller,price.absoluteGoldPrice),5+csize)+"] ^<SHOP^>"+Util.padRight(E.name(),"^</SHOP^>",totalWidth-csize);
                    if((++colNum)>totalCols)
                    {
                        str.append("\n\r");
                        rowNum++;
                                      ...
                                      Environmental product)
    {
        ...
        if((coreSoldItem!=null)&&(shop.doISellThis(coreSoldItem)))
        {
            ...
            String currency=BeanCounter.getCurrency(shopkeeper);
            if(!(shopkeeper instanceof ShopKeeper))
                BeanCounter.subtractMoney(shopkeeper,currency,val);
            BeanCounter.giveSomeoneMoney(shopkeeper,pawner,currency,val);
            pawner.recoverEnvStats();
            pawner.tell(shopkeeper.name()+" pays you "+BeanCounter.nameCurrencyShort(shopkeeper,val)+" for "+rawSoldItem.name()+".");
                                         ...
                                         Environmental product)
    {
        ...
        if(price.absoluteGoldPrice>0.0) 
        {
            BeanCounter.subtractMoney(buyer,BeanCounter.getCurrency(seller),price.absoluteGoldPrice);
            ...
            if(CoffeeShops.getSalesTax(seller.getStartRoom(),seller)!=0.0)
            {
                ...
                if((theLaw!=null)&&(A2!=null))
                {
                    ...
                    if(treasuryR!=null)
                    {
                        ...
                        Coins COIN=BeanCounter.makeBestCurrency(BeanCounter.getCurrency(seller),taxAmount,treasuryR,treasuryItem);
            ...
            if(seller.isMonster())
            {
                ...
                {
                    BeanCounter.modifyLocalBankGold(seller.getStartRoom().getArea(),
                                                    T.landOwner(),
                                                    CoffeeUtensils.getFormattedDate(buyer)+": Deposit of "+BeanCounter.nameCurrencyShort(seller,totalFunds)+": Purchase: "+product.Name()+" from "+seller.Name(),
                                                    BeanCounter.getCurrency(seller),
                                                    totalFunds);
                }
            }
        }
