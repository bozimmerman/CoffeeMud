...
*/
public class StdWearable extends StdItem implements Armor
{
	@Override
	public String ID()
	{
	...
	public StdWearable()
	{
		...
		setName("a bracer");
		setDisplayText("a bracer sits here.");
	...
	protected static String armorHealth(final Armor A)
	{
		final int[] condSet = new int[]{95, 75, 50, 25, 10, 5, 0};
		int ordinal=0;
		for(int i=0;i<condSet.length;i++)
		{
			if(A.usesRemaining()>=condSet[i])
			{
				ordinal=i;
				break;
			}
		...
		switch(A.material()&RawMaterial.MATERIAL_MASK)
		{
		...
		return CMLib.lang().L(message,A.name(),""+A.usesRemaining());
	}
	...
	protected static SizeDeviation getSizingDeviation(final Armor A, final MOB mob)
	{
		if(mob == null)
			return SizeDeviation.FITS;

		if((A.phyStats().height()>0)
		&&(mob.phyStats().height()>0))
		{
			long wcode=A.rawProperLocationBitmap();
			int devianceAllowed=200;
			if((wcode&SMALL_DEVIATION_WEAR_LOCS)>0)
				devianceAllowed=20;
			else
			if((wcode&STRANGE_DEVIATION_WEAR_LOCS)>0)
			{
			...
			{
				if(mob.phyStats().height()<(A.phyStats().height()-devianceAllowed))
	...
	protected static boolean doSizeCheck(final Armor A, final Environmental myHost, final CMMsg msg)
	{
		if((msg.amITarget(A))
		&&(msg.targetMinor()==CMMsg.TYP_WEAR))
		{
			final Armor.SizeDeviation deviation = StdWearable.getSizingDeviation(A, msg.source());
			if(deviation != Armor.SizeDeviation.FITS)
			{
				if(deviation == Armor.SizeDeviation.TOO_LARGE)
				{
					msg.source().tell(CMLib.lang().L("@x1 doesn't fit you -- it's too big.",A.name()));
					return false;
				}
				else
				if(deviation == Armor.SizeDeviation.TOO_SMALL)
				{
					msg.source().tell(CMLib.lang().L("@x1 doesn't fit you -- it's too small.",A.name()));
					return false;
				}
			}
		}
	...
	protected static void handleArmorConditionMsgs(final Armor A, final CMMsg msg)
	{
		if((msg.amITarget(A))
		&&((msg.targetMinor()==CMMsg.TYP_LOOK)||(msg.targetMinor()==CMMsg.TYP_EXAMINE))
		&&(A.subjectToWearAndTear())
		&&(A.usesRemaining()<100)
		&&(CMLib.flags().canBeSeenBy(A,msg.source())))
		{
			msg.source().tell(armorHealth(A));
			return;
		}

		final ItemPossessor owner = A.owner();
		...
		&&(CMLib.dice().rollPercentage()>((A.phyStats().level()/2)+(10*A.phyStats().ability())+(CMLib.flags().isABonusItems(A)?20:0)))
		&&(CMLib.dice().rollPercentage()>(((MOB)owner).charStats().getStat(CharStats.STAT_DEXTERITY))))
		{
			...
			final int oldUses=A.usesRemaining();
			if(weaponDamageType>=0)
			{
				...
				case RawMaterial.MATERIAL_GLASS:
					...
					case Weapon.TYPE_STULTIFYING:
						if(CMLib.dice().rollPercentage()<5)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,20,0));
						break;
					case Weapon.TYPE_BASHING:
					case Weapon.TYPE_NATURAL:
						if(CMLib.dice().rollPercentage()<10)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,10,0));
						break;
					case Weapon.TYPE_PIERCING:
					case Weapon.TYPE_SHOOT:
					case Weapon.TYPE_SLASHING:
				...
				case RawMaterial.MATERIAL_LEATHER:
					...
					case Weapon.TYPE_DISRUPTING:
						if(CMLib.dice().rollPercentage()<50)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,50,0));
						break;
					case Weapon.TYPE_MELTING:
					case Weapon.TYPE_BURNING:
						if(CMLib.dice().rollPercentage()<25)
					...
					case Weapon.TYPE_SHOOT:
						if(CMLib.dice().rollPercentage()<10)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,4,0));
						break;
					case Weapon.TYPE_SLASHING:
						if(CMLib.dice().rollPercentage()<5)
							A.setUsesRemaining(A.usesRemaining()-1);
				...
				case RawMaterial.MATERIAL_METAL:
					...
					case Weapon.TYPE_DISRUPTING:
						if(CMLib.dice().rollPercentage()<35)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,35,0));
						break;
					case Weapon.TYPE_MELTING:
						if(CMLib.dice().rollPercentage()<25)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,15,0));
					...
					case Weapon.TYPE_BURNING:
						...
						break;
					case Weapon.TYPE_BASHING:
						if((A.rawWornCode()==Wearable.WORN_HEAD)
						&&(CMLib.dice().rollPercentage()==1)
						&&(CMLib.dice().rollPercentage()==1)
						&&((msg.value())>10)
						&&(!CMSecurity.isDisabled(CMSecurity.DisFlag.AUTODISEASE)))
						{
							final Ability dA=CMClass.getAbility("Disease_Tinnitus");
							if((dA!=null)&&(owner.fetchEffect(dA.ID())==null)&&(!CMSecurity.isAbilityDisabled(dA.ID())))
								dA.invoke((MOB)owner,owner,true,0);
						}
						if(CMLib.dice().rollPercentage()<5)
							A.setUsesRemaining(A.usesRemaining()-2);
						break;
				...
				case RawMaterial.MATERIAL_SYNTHETIC:
					...
					case Weapon.TYPE_DISRUPTING:
						if(CMLib.dice().rollPercentage()<20)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,25,0));
						break;
					case Weapon.TYPE_BURSTING:
					case Weapon.TYPE_MELTING:
					case Weapon.TYPE_SCRAPING:
					...
					case Weapon.TYPE_SHOOT:
						if(CMLib.dice().rollPercentage()<5)
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,8,0));
						break;
					case Weapon.TYPE_SLASHING:
						break;
					}
				...
				case RawMaterial.MATERIAL_PRECIOUS:
					...
					case Weapon.TYPE_LASERING:
					case Weapon.TYPE_SCRAPING:
						if((CMLib.dice().rollPercentage()<50)&&((A.material()&RawMaterial.MATERIAL_MASK)!=RawMaterial.MATERIAL_PRECIOUS))
							A.setUsesRemaining(A.usesRemaining()-CMLib.dice().roll(1,50,0));
						break;
					case Weapon.TYPE_SONICING:
					case Weapon.TYPE_DISRUPTING:
			...
			if(oldUses!=A.usesRemaining())
				A.recoverPhyStats();

			if((A.usesRemaining()<10)
			&&(oldUses!=A.usesRemaining())
			&&(owner!=null)
			&&(owner instanceof MOB)
			&&(A.usesRemaining()>0))
				((MOB)owner).tell(CMLib.lang().L("@x1 is nearly destroyed! (@x2%)",A.name(),""+A.usesRemaining()));
			else
			...
			&&(owner instanceof MOB))
			{
				...
				A.setUsesRemaining(100);
				msg.addTrailerMsg(CMClass.getMsg(ownerM,null,null,
						CMMsg.MSG_OK_VISUAL,CMLib.lang().L("^I@x1 is destroyed!!^?",A.name()),
						CMMsg.NO_EFFECT,null,
						CMMsg.MSG_OK_VISUAL,CMLib.lang().L("^I@x1 being worn by <S-NAME> is destroyed!^?",A.name())));
				if(A instanceof Container)
					((Container)A).emptyPlease(false);
				A.unWear();
				A.destroy();
				owner.recoverPhyStats();
				ownerM.recoverCharStats();
				ownerM.recoverMaxState();
				if(ownerM.location()!=null)
					ownerM.location().recoverRoomStats();
			}
		}
	}
	...
	protected static void applyMagicArmorBonus(final Armor A, final Physical affected, final PhyStats affectableStats)
	{
		if((!A.amWearingAt(Wearable.IN_INVENTORY))
		&&((!A.amWearingAt(Wearable.WORN_FLOATING_NEARBY))||(A.fitsOn(Wearable.WORN_FLOATING_NEARBY)))
		&&((!A.amWearingAt(Wearable.WORN_HELD))||(A instanceof Shield)))
		{
			affectableStats.setArmor(affectableStats.armor()-A.phyStats().armor());
			if(A.phyStats().armor()!=0)
			{
				final int ability=A.rawLogicalAnd() ? (A.phyStats().ability()*CMath.numberOfSetBits(A.rawWornCode())) : A.phyStats().ability();
				if(A.amWearingAt(Wearable.WORN_TORSO))
					affectableStats.setArmor(affectableStats.armor()-(ability*5));
				else
				if((A.amWearingAt(Wearable.WORN_HEAD))||(A.amWearingAt(Wearable.WORN_HELD)))
					affectableStats.setArmor(affectableStats.armor()-(ability*2));
				else
				if(!A.amWearingAt(Wearable.WORN_FLOATING_NEARBY))
	...
	protected static String getSecretWearableIdentity(final String baseSecret, final Armor A)
	{
		String id;
		if(A.phyStats().ability()>0)
			id=A.name()+" +"+A.phyStats().ability()+((baseSecret.length()>0)?"\n\r":"")+baseSecret;
		else
		if(A.phyStats().ability()<0)
			id=A.name()+" "+A.phyStats().ability()+((baseSecret.length()>0)?"\n\r":"")+baseSecret;
		else
			id=baseSecret;
		final int timsLevel=CMLib.itemBuilder().timsLevelCalculator(A);
		if(timsLevel != A.phyStats().level())
		...
		if(A.amBeingWornProperly())
			A.affectPhyStats(A.owner(),stats);
		else
		{
			synchronized(A)
			{
				final long wornCode=A.rawWornCode();
				try
				{
					A.setRawWornCode(A.rawProperLocationBitmap());
					A.affectPhyStats(A.owner(),stats);
				}
				finally
				{
					A.setRawWornCode(wornCode);
				}
			}
		}
		id +="\n\r"+CMLib.lang().L("Base Protection: @x1",""+(A.phyStats().armor()-stats.armor()));
		return id;
	}
	...
	{
		return getSecretWearableIdentity(super.secretIdentity(), this);
	}
}
