...
public class Wand_Advancement extends Wand implements MiscMagic
{
	...
	public void affect(Affect affect)
	{
		...
		case Affect.SOUND_WORDS:
			   ...
			   &&(mob.location().isInhabitant((MOB)affect.target())))
			{
				...
				if(x>=0)
				{
					if((usesRemaining()>0)&&(useTheWand(new Falling(),mob)))
					{
						this.setUsesRemaining(this.usesRemaining()-1);
						FullMsg msg=new FullMsg(mob,affect.target(),null,Affect.VISUAL_WNOISE,Affect.VISUAL_WNOISE,Affect.VISUAL_WNOISE,"<S-NAME> point(s) "+this.name()+" at <T-NAME>, who begins to glow softly.");
