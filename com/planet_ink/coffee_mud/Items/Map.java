...
public class Map extends StdItem
{
	...
	public StringBuffer getMyMappedRoom()
	{
		...
		Object o=Resources.getResource("map"+envStats().level()+":"+text());
		...
		for(int a=0;a<mapAreas.size();a++)
		{
			...
			for(Enumeration e=MUD.map.elements();e.hasMoreElements();)
			{
				Room room=(Room)e.nextElement();
				if(room.getAreaID().trim().equalsIgnoreCase(area))
				{
					MapRoom m=new MapRoom();
					m.r=room;
					mapRooms.addElement(m);
				}
			}
		}
		...
		myMap=map;
		Resources.submitResource("map"+envStats().level()+":"+text(),myMap);
		return myMap;
	}
