...
public class SMTPclient
{
	...
	*/
	...
	throws IOException
	{
		String rstr;
		String sstr;
		String real_from_address=WebIQ.ini("WEBIQEMAILADDRESS");
		...
		sstr = "MAIL FROM: " + real_from_address ;
		send.print(sstr);
		send.print(EOL);
		send.flush();
		rstr = reply.readLine();
...
//				send.print(EOL);
	...
	*/
    public synchronized void sendMessage(String froaddress, String to_address, String subject, String message,
										 String attachmentFilename, String attachmentText)
	throws IOException
	{
		String rstr;
		String sstr;
		String boundary="------=_NextPart_000_0010_01C3FBD5.693B0FB0";
		String real_from_address=WebIQ.ini("WEBIQEMAILADDRESS");
...
//		send.print(EOL);
		...
		str.append(EOL);
		str.append("        boundary=\""+boundary.substring(2)+"\"");
		str.append(EOL+"Status:"+EOL+EOL+"This is a multi-part message in MIME format."+EOL);
		...
		if(message!=null)
		{
		    ...
		    {
//				String BoundryString="---"+Math.random()+"_"+Math.random();
//				str.append("Content-Type: multipart/mixed; boundry="+BoundryString);
//				str.append(EOL);
//				str.append(BoundryString);
//				str.append(EOL);
			...
			// Sending a blank line ends the header part.
			str.append("Content-Transfer-Encoding: 7bit");
			str.append(EOL);
			str.append(EOL);
			str.append(message);
			str.append(EOL);
		...
		{
			if((attachmentText.indexOf("<HTML>")>=0)&&(attachmentText.indexOf("</HTML>")>=0))
		    {
//				String BoundryString="---"+Math.random()+"_"+Math.random();
//				str.append("Content-Type: multipart/mixed; boundry="+BoundryString);
//				str.append(EOL);
...
//				str.append(EOL);
				...
				{
					str.append("        name=\"webiq.html\"");
					str.append(EOL);
					str.append("Content-Transfer-Encoding: 8bit");
					str.append(EOL);
					str.append("Content-Disposition: attachment;");
					str.append(EOL);
					str.append("        filename=\"webiq.html\"");
					str.append(EOL);
				}
			}
			else
			{
				...
				if(attachmentFilename==null)
				{
					str.append("        name=\"webiq.txt\"");
					str.append(EOL);
					str.append("Content-Transfer-Encoding: 8bit");
					str.append(EOL);
					str.append("Content-Disposition: attachment;");
					str.append(EOL);
					str.append("        filename=\"webiq.txt\"");
					str.append(EOL);
				}
			}
			if(attachmentFilename!=null)
			{
				str.append("        name=\""+attachmentFilename+"\"");
				str.append(EOL);
				str.append("Content-Transfer-Encoding: 8bit");
				str.append(EOL);
				str.append("Content-Disposition: attachment;");
				str.append(EOL);
				str.append("        filename=\""+attachmentFilename+"\"");
				str.append(EOL);
			}
			str.append(EOL);
			str.append(attachmentText);
			str.append(EOL);
		}
		send.print(str.toString());
		send.flush();
	...
	*/
	...
	/** 
	 * Queue up an email for SMTPSend
	 * 
  	 * <br><br><b>Usage:</b> Email(S, WebIQ.GetINIInfo().MAILSERVERIP,S.getMyUser().Email1.trim(), toEmailAddress.trim(),
							   "Company Administrator Created", Message.trim());
	 * @param tS Session object
	 * @param IP The IP address
	 * @param tFrom Who is it from
	 * @param tTo Sho is is to
	 * @param tSubject The subject of the email
	 * @param tMessage The body of the email
	 * @return NA
	 */
	public static void sendEmail(Session tS, String IP, String tFrom, String tTo, String tSubject, 
							 String tMessage)
	{
		SMTPSend QS=new SMTPSend(tS,IP,tFrom,tTo,tSubject,tMessage);
		QS.start();
	}
	 ...
	 */
	public static void sendEmail(Session tS, String IP, String tFrom, String tTo, String tSubject, 
							 String tMessage, String attachFilename, String attachText)
	{
		SMTPSend QS=new SMTPSend(tS,IP,tFrom,tTo,tSubject,tMessage,attachFilename,attachText);
		QS.start();
	}
}
