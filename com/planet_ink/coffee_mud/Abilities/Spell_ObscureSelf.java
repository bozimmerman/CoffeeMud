	...
	implements IllusionistDevotion
{
	...
	public boolean okAffect(Affect affect)
	{
		...
		if(affect.amITarget(mob))
		{
			...
			if((!affect.amISource(mob))&&(affect.targetCode()==Affect.VISUAL_LOOK)||(affect.targetCode()==Affect.VISUAL_READ))
			{
	...
	public boolean invoke(MOB mob, Vector commands)
	{
		...
		{
			FullMsg msg=new FullMsg(mob,target,this,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,"<S-NAME> fizzles a spell, since "+mob.charStats().heshe()+" is affected by that spell.");
		...
		if(success)
		{
			...
			// what happened.
			invoker=mob;
			FullMsg msg=new FullMsg(mob,target,this,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,"<S-NAME> whisper(s) to <S-HIS-HER>self.");
			if(mob.location().okAffect(msg))
			{
				mob.location().send(mob,msg);
			...
			return beneficialFizzle(mob,target,"<S-NAME> whispers(s) to <S-HIS-HER>self, but nothing happens.");
