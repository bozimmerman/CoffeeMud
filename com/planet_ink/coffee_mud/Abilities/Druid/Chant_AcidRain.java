...
public class Chant_AcidRain extends Chant
{
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		if((affected!=null)&&(affected instanceof Room))
		{
			...
			for(int i=0;i<R.numInhabitants();i++)
			{
				...
				if((M!=null)&&(CMLib.dice().rollPercentage()>M.charStats().getSave(CharStats.STAT_SAVE_ACID)))
					CMLib.combat().postDamage(invoker(),M,this,CMLib.dice().roll(1,M.envStats().level(),1),CMMsg.MASK_GENERAL|CMMsg.TYP_ACID,Weapon.TYPE_MELTING,"The acid rain <DAMAGE> <T-NAME>!");
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			// what happened.
			invoker=mob;
			CMMsg msg=CMClass.getMsg(mob,null,this,affectType(auto),auto?"":"^S<S-NAME> chant(s) to the rain.^?");
