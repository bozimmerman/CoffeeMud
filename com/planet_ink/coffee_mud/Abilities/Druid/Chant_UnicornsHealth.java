...
public class Chant_UnicornsHealth extends Chant
{
	...
	public void affectCharStats(MOB affected, CharStats affectableStats)
	{
		...
		affectableStats.setStat(CharStats.STAT_CONSTITUTION,affectableStats.getStat(CharStats.STAT_CONSTITUTION)+((affected.envStats().level()+(2*super.getXLEVELLevel(invoker())))/6));
	}
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			// what happened.
			invoker=mob;
			CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?"<S-NAME> gain(s) the health of a unicorn!":"^S<S-NAME> chant(s), feeling the health of a unicorn!^?");
