...
*/
public class Fighter_PlanarMount extends FighterSkill
{
	@Override
	public String ID()
	{
		return "Fighter_PlanarMount";
	}

	private final static String localizedName = CMLib.lang().L("Planar Mount");
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg)
	{
		...
		&&(CMLib.flags().getPlaneOfExistence(msg.source().location())!=null))
		{
			final Triad<MOB,Rideable,int[]> mnt;
			synchronized(this)
			{
				...
				||(choices.containsFirst(targM.baseCharStats().getMyRace().racialCategory())))
			{
				...
				&&(R!=null))
				{
					...
					((MOB)mR).baseCharStats().setStat(CharStats.STAT_INTELLIGENCE, 1);
					((MOB)mR).charStats().setStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ, 0);
					((MOB)mR).baseCharStats().setStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ, 0);
					((MOB)mR).charStats().setStat(CharStats.STAT_INTELLIGENCE, 1);
