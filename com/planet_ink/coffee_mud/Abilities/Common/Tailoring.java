...
public class Tailoring extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill
{
	...
	private final static String localizedName = CMLib.lang()._("Tailoring");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(commands.size()==0)
		{
			commonTell(mob,_("Knit what? Enter \"knit list\" for a list, \"knit refit <item>\" to resize, \"knit learn <item>\", \"knit scan\", \"knit mend <item>\", or \"knit stop\" to cancel."));
			return false;
		}
		if((!auto)
		&&(commands.size()>0)
		...
		else
		{
			...
			if(foundRecipe==null)
			{
				commonTell(mob,_("You don't know how to knit a '@x1'.  Try \"@x2 list\" for a list.",recipeName,triggerStrings()[0].toLowerCase()));
				return false;
			}
			...
			startStr="<S-NAME> start(s) knitting "+buildingI.name()+".";
			displayText="You are knitting "+buildingI.name();
			verb="knitting "+buildingI.name();
