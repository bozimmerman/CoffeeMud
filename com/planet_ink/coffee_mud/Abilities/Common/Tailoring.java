...
public class Tailoring extends EnhancedCraftingSkill implements ItemCraftor
{
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(str.equalsIgnoreCase("list"))
		{
			...
			for(int r=0;r<toggleTop;r++)
				buf.append(CMStrings.padRight("Item",15)+" Lvl "+CMStrings.padRight("Cloth",5)+" ");
			buf.append("\n\r");
			for(int r=0;r<recipes.size();r++)
			{
				...
				if(V.size()>0)
				{
					...
					{
						buf.append(CMStrings.padRight(item,15)+" "+CMStrings.padRight(""+level,3)+" "+CMStrings.padRight(""+wood,5)+((toggler!=toggleTop)?" ":"\n\r"));
		...
		else
		{
			...
			completion=CMath.s_int((String)foundRecipe.elementAt(Tailoring.RCP_TICKS))-((mob.envStats().level()-CMath.s_int((String)foundRecipe.elementAt(RCP_LEVEL)))*2);
