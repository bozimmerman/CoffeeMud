...
public class Armorsmithing extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill
{
	...
	protected boolean autoGenInvoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel, final int autoGenerate, final boolean forceLevels, final List<CraftedItem> crafted)
	{
		...
		if(str.equalsIgnoreCase("refit"))
		{
			...
			{
				commonFaiL(mob,commands,"That's not made of metal.  That can't be refitted.");
				return false;
			}
		...
		else
		{
			...
			if(foundRecipe==null)
			{
				commonFaiL(mob,commands,"You don't know how to make a '@x1'.  Try \"armorsmith list\" for a list.",recipeName);
				return false;
			}
