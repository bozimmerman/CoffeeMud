...
public class Construction extends CraftingSkill
{
	...
	private final static String localizedName = CMLib.lang()._("Construction");
			...
			if((affected!=null)&&(affected instanceof MOB)&&(!helping))
			{
				...
				if(!aborted)
				{
					...
					case BUILD_WALL:
						...
						break;
					case BUILD_DOOR:
						commonTell(mob,_("You've ruined the door!"));
						break;
					case BUILD_SECRETDOOR:
						commonTell(mob,_("You've ruined the secret door!"));
						break;
					case BUILD_FENCE:
						commonTell(mob,_("You've ruined the fence!"));
						break;
					case BUILD_GATE:
						commonTell(mob,_("You've ruined the gate!"));
						break;
					case BUILD_TITLE:
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(commands.size()==0)
		{
			commonTell(mob,_("Construct what, where? Try Construct list."));
			return false;
		}
		...
		if(("LIST").startsWith(str.toUpperCase()))
		{
			...
			final StringBuffer buf=new StringBuffer(_("@x1 Wood required\n\r",CMStrings.padRight(_("Item"),colWidth)));
		...
		if(str.equalsIgnoreCase("help"))
		{
			...
			{
				commonTell(mob,_("@x1 is not constructing anything.",targetMOB.Name()));
				return false;
			}
		...
		if(doingCode<0)
		{
			commonTell(mob,_("'@x1' is not a valid construction project.  Try LIST.",firstWord));
			return false;
		}
		...
		if(doingCode==BUILD_DESC)
		{
			if(commands.size()<3)
			{
				commonTell(mob,_("You must specify an exit direction or room, followed by a description for it."));
				return false;
			}
		...
		default:
			if(dir<0)
			{
				...
				{
					commonTell(mob,null,null,_("There are no wooden constructs to demolish here!"));
					return false;
				}
				else
				if(CMLib.law().isHomeRoomUpstairs(mob.location()))
