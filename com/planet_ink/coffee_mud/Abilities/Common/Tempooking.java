...
public class Tempooking extends CraftingSkill implements ItemCraftor
{
	@Override
	public String ID()
	{
	...
	private final static String	localizedName	= CMLib.lang().L("Tempooking");
	...
	private static final String[]	triggerStrings	= I(new String[] { "TEMPOOK", "TEMPOOKING" });
	...
	public Map<String,Integer> potContents(Container pot)
	{
		...
		if((pot instanceof Drink)&&(((Drink)pot).liquidRemaining()>0))
		{
			...
			Log.debugOut("pot: "+h.keySet().iterator().next()+", "+h.get(h.keySet().iterator().next()));
		}
		if(pot.owner()==null)
			return h;
		...
		for(int v=0;v<V.size();v++)
		{
			...
			Log.debugOut("deep content: "+I.name());
			String ing="Unknown";
			...
			Log.debugOut("pot: "+ing+"/"+I.secretIdentity().toUpperCase()+"/"+I.Name().toUpperCase()+"/"+INT);
		}
		Log.debugOut("/potContents");
		return h;
	}
	...
	{
		Log.debugOut("countIngredients: "+Vr.size());
		...
		{
			Log.debugOut("RanOutOfSomething: "+RanOutOfSomething+", NotEnoughForThisRun:"+NotEnoughForThisRun+", amountMade: "+amountMade);
			for(int vr=RCP_MAININGR;vr<Vr.size();vr+=2)
			{
				final String ingredient=Vr.get(vr).toUpperCase();
				Log.debugOut("ingredient: "+ingredient);
				if(ingredient.length()>0)
				{
					...
					for(int i=0;i<contents.length;i++)
					{
						...
						Log.debugOut("ingredient2: "+ingredient2+", amount2:"+amount2+", index:"+index);
		...
		if(NotEnoughForThisRun)
		{
			...
			Log.debugOut("add to codedList: "+(-amountMade));
			for(int i=0;i<contents.length;i++)
			{
				if(amounts[i]<0)
				{
					...
					Log.debugOut("add to codedList: "+content);
		...
		{
			codedList.addElement(Integer.valueOf(amountMade));
			Log.debugOut("add codedList: "+amountMade);
			for(int i=0;i<contents.length;i++)
			{
				final String ingredient2=contents[i];
				final int amount2=amounts[i];
				Log.debugOut("check ingredient2: "+ingredient2+", amount2: "+amount2);
				if((amount2>0)
				...
				&&(!ingredient2.toUpperCase().startsWith("WATER/")))
				{
					...
					if(content.indexOf('/')>=0)
						content=content.substring(0,content.indexOf('/'));
					Log.debugOut("add codedList: "+content);
					codedList.addElement(content);
				}
			}
		}
	...
	{
		Log.debugOut("extraIngredientsInOldContents: "+perfectOnly+"/"+Vr.size());
		...
		for(final String ingredient : oldPotContents.keySet())
		{
			...
			{
				found=true;
				Log.debugOut("Found: ingredient: "+ingredient);
			}
			else
			{
				Log.debugOut("ingredient: "+ingredient);
				for(int vr=RCP_MAININGR;vr<Vr.size();vr+=2)
				{
					...
					{
						found=true;
						Log.debugOut("Found: ingredient2: "+ingredient2+", index="+index);
					}
					else
						Log.debugOut("ingredient2: "+ingredient2+", index="+index);
				}
			}
			if(!found)
			{
				...
				if(content.indexOf('/')>=0)
					content=content.substring(0,content.indexOf('/'));
				Log.debugOut("extra content: "+content);
	...
	{
		Log.debugOut("missingIngredientsFromOldContents: "+perfectOnly+"/"+Vr.size());
		...
		for(int vr=RCP_MAININGR;vr<Vr.size();vr+=2)
		{
			...
			if(ingredient.length()>0)
			{
				...
				if(vr<Vr.size()-1)
					amount=CMath.s_int(Vr.get(vr+1));
				Log.debugOut("ingredient: "+ingredient+", amount: "+amount);
				boolean found=false;
				for(final String ingredient2 : oldPotContents.keySet())
				{
					...
					||((!perfectOnly)&&ingredient2.equalsIgnoreCase(ingredient.toUpperCase())))
					{
						...
						Log.debugOut("not found: ingredient2: "+ingredient2+", index: "+index);
				}
				if(amount>=0)
				{
					if(!found)
					{
						Log.debugOut("missing"+ingredient);
		...
		{
			Log.debugOut("missing: "+possiblyMissing);
									...
									final int asLevel, int autoGenerate, boolean forceLevels, List<Item> crafted)
	{
		...
		Log.debugOut("target="+target.name());
		for(int a=0;a<mob.numEffects();a++)
		{
			final Ability A=mob.fetchEffect(a);
		...
		Log.debugOut("RECIPE FIND START");
		for(int v=0;v<allRecipes.size();v++)
		{
			final List<String> Vr=allRecipes.get(v);
			Log.debugOut("recipe #"+v+", name="+Vr.get(0).toString());
			boolean found=false;
			for(final String ingredient2 : oldPotContents.keySet())
			{
			...
			if(found)
			{
				Log.debugOut("it's close!");
				closeRecipes.addElement(Vr);
			}
			else
				Log.debugOut("NOT close!");
		...
		if(perfectRecipes.size()==0)
		{
			...
			Log.debugOut("CHECKING CLOSE RECIPES");
