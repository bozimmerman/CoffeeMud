...
public class MasterCostuming extends CraftingSkill
{
	...
	public boolean canBeLearnedBy(MOB teacher, MOB student)
	{
		...
		{
			teacher.tell(student.name()+" has not yet learned costuming.");
			student.tell("You need to learn costuming before you can learn "+name()+".");
			return false;
		}
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(str.equalsIgnoreCase("list"))
		{
			...
			for(int r=0;r<toggleTop;r++)
				buf.append(Util.padRight("Item",27)+" Lvl "+Util.padRight("Cloth",6)+" ");
			buf.append("\n\r");
			for(int r=0;r<recipes.size();r++)
			{
				...
				if(V.size()>0)
				{
					...
					{
						buf.append(Util.padRight(item,27)+" "+Util.padRight(""+level,3)+" "+Util.padRight(""+wood,6)+((toggler!=toggleTop)?" ":"\n\r"));
		...
		else
		{
			...
			if(foundRecipe==null)
			{
				commonTell(mob,"You don't know how to costume a '"+recipeName+"'.  Try \"mcostume list\" for a list.");
				return false;
			}
