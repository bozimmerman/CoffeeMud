...
public class Sculpting extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill
{
	...
	public boolean invoke(final MOB mob, Vector commands, Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(str.equalsIgnoreCase("list"))
		{
			...
			StringBuffer buf=new StringBuffer(CMStrings.padRight("Item",cols[0])+" "+CMStrings.padRight("Lvl",cols[1])+" Stone required\n\r");
		...
		else
		{
			...
			int lostValue=autoGenerate>0?0:
				CMLib.materials().destroyResources(mob.location(),woodRequired,data[0][FOUND_CODE],0,buildingI)
