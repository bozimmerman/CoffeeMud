...
public class Sculpting extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill
{
								 ...
								 final int asLevel, final int autoGenerate, final boolean forceLevels, final List<CraftedItem> crafted)
	{
		...
		else
		{
			...
			if(foundRecipe==null)
			{
				commonFaiL(mob,commands,"You don't know how to sculpt a '@x1'.  Try \"sculpt list\" for a list.",recipeName);
				return false;
			}
			...
			{
				commonFaiL(mob,commands,"You are not allowed to build that here.");
				return false;
			}
			if(!super.invoke(mob,commands,givenTarget,auto,asLevel))
				return false;
