package com.planet_ink.coffee_mud.Abilities;

import com.planet_ink.coffee_mud.interfaces.*;
import com.planet_ink.coffee_mud.StdAffects.*;
import com.planet_ink.coffee_mud.utils.*;
import com.planet_ink.coffee_mud.Locales.*;
import com.planet_ink.coffee_mud.service.*;
import com.planet_ink.coffee_mud.commands.*;
import com.planet_ink.coffee_mud.CharClasses.*;
import java.util.*;

public class Ranger_Track extends StdAbility
{
	private MOB trackingWhom=null;
	private Vector theTrail=null;
	private Hashtable lookedIn=null;
	public int nextDirection=-2;

	public Ranger_Track()
	{
		super();
		myID=this.getClass().getName().substring(this.getClass().getName().lastIndexOf('.')+1);
		name="Track";
		displayText="(tracking)";
		miscText="";
		triggerStrings.addElement("TRACK");

		malicious=false;
		baseEnvStats().setLevel(1);

		addQualifyingClass(new Ranger().ID(),1);

		canBeUninvoked=true;
		isAutoinvoked=false;
	}

	public int classificationCode()
	{
		return Ability.SKILL;
	}

	public Environmental newInstance()
	{
		return new Ranger_Track();
	}

	public int nextDirectionFromHere(Room location)
	{
		if(theTrail==null)
			return -1;
		if(location==theTrail.elementAt(0))
			return 999;

		Room nextRoom=null;
		int bestDirection=-1;
		int trailLength=Integer.MAX_VALUE;
		for(int dirs=0;dirs<6;dirs++)
		{
			Room thisRoom=location.getRoom(dirs);
			Exit thisExit=location.getExit(dirs);
			if((thisRoom!=null)&&(thisExit!=null))
			{
				if(thisRoom instanceof GridLocale)
					thisRoom=((GridLocale)thisRoom).getAltRoomFrom(location);
				if(thisRoom!=null)
					for(int trail=0;trail<theTrail.size();trail++)
					{
						if((theTrail.elementAt(trail)==thisRoom)
						&&(trail<trailLength))
						{
							bestDirection=dirs;
							trailLength=trail;
							nextRoom=thisRoom;
						}
					}
			}
		}
		return bestDirection;
	}

	public boolean tick(int tickID)
	{
		if(!super.tick(tickID))
			return false;
		if(tickID==ServiceEngine.MOB_TICK)
		{
			if(nextDirection==-999)
				return true;

			if((theTrail==null)
			||(affected == null)
			||(!(affected instanceof MOB)))
				return false;

			MOB mob=(MOB)affected;

			if(nextDirection==999)
			{
				mob.tell("The trail seems to pause here.");
				nextDirection=-2;
				unInvoke();
			}
			else
			if(nextDirection==-1)
			{
				mob.tell("The trail dries up here.");
				nextDirection=-999;
				unInvoke();
			}
			else
			if(nextDirection>=0)
			{
				mob.tell("The trail seems to continue "+Directions.getDirectionName(nextDirection)+".");
				nextDirection=-2;
			}

		}
		return true;
	}

	public void affect(Affect affect)
	{
		super.affect(affect);

		if((affected==null)||(!(affected instanceof MOB)))
			return;

		MOB mob=(MOB)affected;
		if((affect.amISource(mob))&&(affect.amITarget(mob.location()))&&(Sense.canBeSeenBy(mob.location(),mob)))
		{
			switch(affect.targetCode())
			{
			case Affect.VISUAL_LOOK:
				nextDirection=nextDirectionFromHere(mob.location());
				break;
			}
		}
	}

	public boolean findTheBastard(Room location, String mobName)
	{
		if(lookedIn==null) return false;
		if(lookedIn.get(location)!=null)
			return false;
		lookedIn.put(location,location);
		for(int i=0;i<6;i++)
		{
			Room nextRoom=location.getRoom(i);
			Exit nextExit=location.getExit(i);
			if((nextRoom!=null)&&(nextExit!=null))
			{
				MOB mobCheck=nextRoom.fetchInhabitant(mobName);
				if((mobCheck!=null)
				||(findTheBastard(nextRoom,mobName)))
				{
					if(mobCheck!=null)
						trackingWhom=mobCheck;

					if(theTrail==null)
						theTrail=new Vector();
					theTrail.addElement(nextRoom);
					return true;
				}
			}
		}
		return false;
	}

	public boolean invoke(MOB mob, Vector commands)
	{
		if(!Sense.canPerformAction(mob))
			return false;

		if(!Sense.canBeSeenBy(mob.location(),mob))
		{
			mob.tell("You can't see anything to track!");
			return false;
		}

		Ranger_Track oldTrack=(Ranger_Track)mob.fetchAffect(this.ID());
		if(oldTrack!=null)
		{
			mob.tell(mob,null,"You stop tracking "+trackingWhom.name()+".");
			oldTrack.unInvoke();
		}

		trackingWhom=null;
		theTrail=null;
		nextDirection=-2;

		if(!super.invoke(mob,commands))
			return false;

		String mobName=CommandProcessor.combine(commands,0);
		if(mobName.length()==0)
		{
			mob.tell("Track whom?");
			return false;
		}

		if(mob.location().fetchInhabitant(mobName)!=null)
		{
			mob.tell("Try 'look'.");
			return false;
		}

		boolean success=profficiencyCheck(0);

		lookedIn=new Hashtable();
		findTheBastard(mob.location(),mobName);

		if((success)&&(trackingWhom!=null)&&(theTrail!=null))
		{
			theTrail.addElement(mob.location());
			MOB target=trackingWhom;
			// it worked, so build a copy of this ability,
			// and add it to the affects list of the
			// affected MOB.  Then tell everyone else
			// what happened.
			FullMsg msg=new FullMsg(mob,target,this,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,"<S-NAME> begin(s) to track <T-NAME>.");
			if(mob.location().okAffect(msg))
			{
				mob.location().send(mob,msg);
				invoker=mob;
				displayText="(tracking "+trackingWhom.name()+")";
				Ranger_Track newOne=(Ranger_Track)this.copyOf();
				mob.addAffect(newOne);
				mob.recoverEnvStats();
				newOne.nextDirection=newOne.nextDirectionFromHere(mob.location());
			}
		}
		else
			return beneficialFizzle(mob,null,"<S-NAME> attempt(s) to track "+mobName+", but can't find the trail.");


		// return whether it worked
		return success;
	}

}