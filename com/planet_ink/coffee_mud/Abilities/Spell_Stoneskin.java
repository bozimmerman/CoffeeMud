	...
	public boolean okAffect(Affect affect)
	{
		...
		if(affect.amITarget(mob))
		{
			...
			case Affect.STRIKE_HANDS:
				mob.location().show(mob,affect.source(),Affect.VISUAL_WNOISE,"The stone skin around <S-NAME> absorbs the attack from <T-NAME>.");
				if((--HitsRemaining)<=0)
					unInvoke();
				return false;
			default:
	...
	public boolean invoke(MOB mob, Vector commands)
	{
		...
		if(success)
		{
			...
			if(mob.location().okAffect(msg))
			{
				...
				mob.location().show(mob,target,Affect.VISUAL_WNOISE,"<T-NAME> watch(es) <T-HIS-HER> skin turn hard as stone!");
