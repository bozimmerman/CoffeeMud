...
public class Prayer_ProtGood extends Prayer
{
	...
	public boolean tick(int tickID)
	{
		...
		{
			mob.location().show(mob,null,Affect.VISUAL_WNOISE,"The protective aura of goodness around <S-NAME> "+TheFight.hitWord(-1,mob.envStats().level())+" <S-HIS-HER> skin!");
	...
	public boolean okAffect(Affect affect)
	{
		...
		if(affect.target()==invoker)
		{
			...
			&&(affect.tool() instanceof Prayer))
			{
				...
				{
					affect.source().location().show(invoker,null,Affect.VISUAL_WNOISE,"The unholy field around <S-NAME> protect(s) <S-HIS-HER> from the goodly magic attack of "+affect.source().name()+".");
					return false;
				}
			}
	...
	public boolean invoke(MOB mob, Vector commands)
	{
		...
		if(success)
		{
			...
			// what happened.
			FullMsg msg=new FullMsg(mob,null,this,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,Affect.SOUND_MAGIC,"<S-NAME> call(s) upon the protection of <S-HIS-HER> god from goodness.");
