...
public class Thief_ConcealDoor extends ThiefSkill
{
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
    {
        ...
        if(success)
        {
            CMMsg msg=CMClass.getMsg(mob,E,null,CMMsg.MSG_THIEF_ACT,"<S-NAME> coneal(s) <T-NAME>.",CMMsg.MSG_THIEF_ACT,null,CMMsg.MSG_THIEF_ACT,null);
            if(mob.location().okMessage(mob,msg))
            {
                ...
                ||((R2!=null)&&(CMLib.utensils().doesOwnThisProperty(mob,R2))))
                {
                    ...
                    A.startTickDown(mob,E,Integer.MAX_VALUE/2);
                E.recoverEnvStats();
            }
        }
        else
