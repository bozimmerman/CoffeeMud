...
public class TrapDisablingThiefSkill extends ThiefSkill {
	public String ID() { return "TrapDisablingThiefSkill"; }
	public String name(){ return "a trap disabling Thief Skill";}
    private static final int EXPERTISE_STAGES=10;
    private static final String[] EXPERTISE={"DETRAP"};
    private static final String[] EXPERTISE_NAME={"Trap Disabling"};
        ...
        if(CMLib.expertises().getDefinition(EXPERTISE[0]+EXPERTISE_STAGES)==null)
            for(int i=1;i<=EXPERTISE_STAGES;i++)
                CMLib.expertises().addDefinition(EXPERTISE[0]+i,EXPERTISE_NAME[0]+" "+CMath.convertToRoman(i),
                        "","+DEX "+(13+i)+" -LEVEL +>="+(12+(5*i)),0,1,0,0,0);
        if(!ID().equals("TrapDisablingThiefSkill"))
