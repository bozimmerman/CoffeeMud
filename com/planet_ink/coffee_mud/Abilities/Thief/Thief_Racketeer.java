...
/* 
	Copyright 2000-2006 Bo Zimmerman, Lee Fox
...
public class Thief_Racketeer extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget,
		...
		if ((target == null) && (givenTarget != null)
		...
		if (A != null) {
			if (A.invoker() == mob)
				mob.tell(target.name()
						+ " has already been extracted from today.");
			else {
				mob.tell(target.name() + " is already under "
						+ A.invoker().name() + "'s protection.");
				A.invoker().tell(
						"Word on the street is that " + mob.name()
								+ " is trying to push into your business with "
								+ target.name() + ".");
			}
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					"<S-NAME> extract(s) "
							+ CMLib.beanCounter().nameCurrencyShort(target,
									amount)
							+ " of protection money from <T-NAME>.");
						...
						new Long(
								((MudHost.TIME_MILIS_PER_MUDHOUR
										* mob.location().getArea().getTimeObj()
										...
										.getDaysInMonth()) / Tickable.TIME_TICK))
								.intValue());
			...
			maliciousFizzle(mob, target,
					"<T-NAME> seem(s) unintimidated by <S-NAME>.");
		return success;
	}
