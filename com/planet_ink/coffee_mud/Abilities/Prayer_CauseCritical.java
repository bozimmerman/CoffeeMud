...
public class Prayer_CauseCritical extends Prayer
{
	...
	public boolean invoke(MOB mob, Vector commands)
	{
		...
		if(success)
		{
			...
			// what happened.
			FullMsg msg=new FullMsg(mob,target,this,Affect.SOUND_MAGIC,Affect.STRIKE_MAGIC,Affect.SOUND_MAGIC,"<S-NAME> pray(s) at <T-NAME> for a critical burst of pain!");
			if(mob.location().okAffect(msg))
			{
				mob.location().send(mob,msg);
				if(!msg.wasModified())
				{
					int harming=Dice.roll(mob.envStats().level(),4,(mob.envStats().level()*4));
