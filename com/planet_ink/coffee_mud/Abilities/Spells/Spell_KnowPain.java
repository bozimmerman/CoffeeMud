...
public class Spell_KnowPain extends Spell
{
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto)
	{
		...
		if(success)
		{
			...
			// what happened.
			FullMsg msg=new FullMsg(mob,target,this,affectType(auto),(auto?"A painful memory erupts!":"^S<S-NAME> invoke(s) <T-NAMEPOSS> most painful memories."));
			...
			if((mob.location().okAffect(msg))&&(mob.location().okAffect(msg2)))
			{
				...
				int damage = 0;
				int maxDie = (target.envStats().level()+adjustedLevel(mob))/4;
				...
				if(target.location()==mob.location())
					ExternalPlay.postDamage(mob,target,this,damage,Affect.MASK_GENERAL|Affect.TYP_COLD,Weapon.TYPE_BURSTING,"The horrible memory <DAMAGE> <T-NAME>!");
			}
		}
		else
