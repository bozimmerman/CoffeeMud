...
*/
public class Spell_TapScroll extends Spell
{
	@Override
	public String ID()
	{
		return "Spell_TapScroll";
	}

	private final static String localizedName = CMLib.lang().L("Tap Scroll");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			mob.tell(L("You can't tap into that."));
			return false;
		}

		if(((Scroll)target).getSpells().size()==0)
		{
			mob.tell(L("That lacks any arcane magic to tap into."));
			return false;
		}
		...
		if(!isArcane)
		{
			mob.tell(L("That does not appear to be an arcane scroll."));
			return false;
		}

		if(target.usesRemaining()<=0)
		{
			mob.tell(L("That is too depleted to tap into."));
			return false;
		}
		...
		if((target!=null)&&(success))
		{
			...
			if(R==null)
				return false;
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?L("<T-NAME> become(s) tapped into."):L("^S<S-NAME> tap(s) into the magic inside <T-NAMESELF>.^?"));
			if(R.okMessage(mob,msg))
			{
				...
				beneficialWordsFizzle(mob,target,auto?"":L("<S-NAME> attempt(s) to tap into the magic of <T-NAMESELF>, but fail(s)."));
		}
