                ...
                if(mob.location().okMessage(mob,msg2))
                {
					...
					if(amountRemaining<0)
					{
						...
						for(int i=0;i<room.numInhabitants();i++)
						{
							...
							&&(M.rangeToTarget()<3)
							&&(!M.amDead()))
								CMLib.combat().postDamage(invoker,M,this,CMLib.dice().roll((M.envStats().level()+getXLEVELLevel(invoker()))/2,6,0),CMMsg.MSG_OK_VISUAL,Weapon.TYPE_PIERCING,"A shard of ice <DAMAGE> <T-NAME>!");
						}
