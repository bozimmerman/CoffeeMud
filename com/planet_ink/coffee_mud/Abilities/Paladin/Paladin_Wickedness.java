...
*/
public class Paladin_Wickedness extends PaladinSkill
{
	@Override
	public String ID()
	{
		return "Paladin_Wickedness";
	}

	private final static String localizedName = CMLib.lang().L("Paladin`s Wickedness");
			...
			&&(appropriateToMyFactions(invoker)))
			{
				for(int m=0;m<R.numInhabitants();m++)
				{
					...
					&&((invoker==null)||(invoker.fetchAbility(ID())==null)||proficiencyCheck(null,0,false)))
					{
						...
						{
							CMLib.combat().postDamage(invoker,target,this,harming,
								CMMsg.MASK_MALICIOUS|CMMsg.MASK_ALWAYS|CMMsg.TYP_CAST_SPELL,Weapon.TYPE_BURSTING,
								L("^SThe aura of wickedness around <S-NAME> <DAMAGES> <T-NAME>!^?"));
						}
					}
				}
			}
