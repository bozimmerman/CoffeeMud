...
public class Prayer_AnimateZombie extends Prayer
{
	...
	private final static String localizedName = CMLib.lang()._("Animate Zombie");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?"":_("^S<S-NAME> @x1 to animate <T-NAMESELF> as a zombie.^?",prayForWord(mob)));
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				final MOB newMOB=CMClass.getMOB("GenUndead");
				newMOB.setName(_("@x1 zombie",race));
				newMOB.setDescription(description);
				newMOB.setDisplayText(_("@x1 zombie is here",race));
