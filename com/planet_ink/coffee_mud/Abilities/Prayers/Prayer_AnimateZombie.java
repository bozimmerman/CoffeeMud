...
public class Prayer_AnimateZombie extends Prayer
{
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?"":"^S<S-NAME> "+prayForWord(mob)+" to animate <T-NAMESELF> as a zombie.^?");
			if(mob.location().okMessage(mob,msg))
			{
				...
				newMOB.baseEnvStats().setLevel(2+(super.getX1Level(mob)*2)+super.getXLEVELLevel(mob));
