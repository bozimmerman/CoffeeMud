...
public class Prayer_FreezeMetal extends Prayer
{
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		...
		for(int i=0;i<mob.inventorySize();i++)
		{
			   ...
			   &&(!mob.amDead()))
			{
				...
				CMLib.combat().postDamage(invoker,mob,this,damage,CMMsg.MASK_GENERAL|CMMsg.TYP_COLD,Weapon.TYPE_BURSTING,item.name()+" <DAMAGE> <T-NAME>!");
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			// what happened.
			invoker=mob;
			CMMsg msg=CMClass.getMsg(mob,target,this,somanticCastCode(mob,target,auto),auto?"":"^S<S-NAME> point(s) at <T-NAMESELF> and "+prayWord(mob)+".^?");
