...
public class Prayer_ProtUndead extends Prayer
{
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		...
		if(mob.charStats().getMyRace().racialCategory().equalsIgnoreCase("Undead"))
		{
			...
			CMLib.combat().postDamage(invoker,mob,this,damage,CMMsg.MASK_GENERAL|CMMsg.TYP_CAST_SPELL,Weapon.TYPE_BURSTING,"^S<T-HIS-HER> protective aura <DAMAGE> <T-NAME>!^?");
		}
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			// what happened.
			CMMsg msg=CMClass.getMsg(mob,target,this,affectType(auto),auto?"<T-NAME> become(s) protected from the undead.":"^S<S-NAME> "+prayWord(mob)+" for protection from the undead.^?");
