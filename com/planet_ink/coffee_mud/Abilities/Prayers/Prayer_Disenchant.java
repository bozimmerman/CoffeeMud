...
public class Prayer_Disenchant extends Prayer
{
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		boolean success=proficiencyCheck(mob,5-((mob.envStats().level()-target.envStats().level())*5),auto);
		...
		if(success)
		{
			...
			// what happened.
			CMMsg msg=CMClass.getMsg(mob,target,this,somanticCastCode(mob,target,auto),auto?"<T-NAME> appear(s) neutralized!":"^S<S-NAME> "+prayForWord(mob)+" to neutralize <T-NAMESELF>.^?");
