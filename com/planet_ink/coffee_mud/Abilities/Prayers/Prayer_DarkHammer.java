...
*/
public class Prayer_DarkHammer extends Prayer
{
	@Override
	public String ID()
	{
		return "Prayer_DarkHammer";
	}

	private final static String localizedName = CMLib.lang().L("Hammer of Darkness");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		&&(CMLib.flags().isEvil(target)))
		{
			...
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto)|CMMsg.MASK_MALICIOUS,
					L(auto?L("<T-NAME> is filled with darkness!"):
						L("^S<S-NAME> @x1 for @x2 of darkness to strike <T-NAMESELF>!^?@x3",
			...
			if((R.okMessage(mob,msg))&&((R.okMessage(mob,msg2))))
			{
				...
				if(CMLib.flags().isGood(target))
				{
					...
					CMLib.combat().postDamage(mob,target,this,harming,CMMsg.MASK_ALWAYS|CMMsg.TYP_JUSTICE,Weapon.TYPE_BURSTING,
							L("^S@x1 of darkness <DAMAGE> <T-NAME>!^?",Wname));
				}
			}
		}
		else
