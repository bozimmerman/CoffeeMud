...
public class Prayer_Godstrike extends Prayer
{
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if((success)&&(CMLib.flags().isEvil(target)))
		{
			...
			CMMsg msg=CMClass.getMsg(mob,target,newOne,somanticCastCode(mob,target,auto)|CMMsg.MASK_MALICIOUS,(auto?"<T-NAME> is filled with holy fury!":"^S<S-NAME> "+prayWord(mob)+" for power against the evil inside <T-NAMESELF>!^?")+CMProps.msp("spelldam1.wav",40));
			if(mob.location().okMessage(mob,msg))
			{
				...
				if(msg.value()<=0)
				{
					...
					if(CMLib.flags().isEvil(target))
						CMLib.combat().postDamage(mob,target,this,harming,CMMsg.MASK_GENERAL|CMMsg.TYP_CAST_SPELL,Weapon.TYPE_BURSTING,"^SThe holy STRIKE of the gods <DAMAGE> <T-NAME>!^?");
				}
			}
		}
		else
