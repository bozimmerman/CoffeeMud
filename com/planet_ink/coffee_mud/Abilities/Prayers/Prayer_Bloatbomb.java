		...
		if(room != null)
		{
			...
			for(final Enumeration<MOB> m=room.inhabitants();m.hasMoreElements();)
			{
				...
				if((M!=null)&&(!friendlySet.contains(M)))
				{
					...
					final int damage=CMLib.dice().roll(1,3+(invoker.phyStats().level()/5),0);
					CMLib.combat().postDamage(invoker,M,null,damage,CMMsg.MASK_MALICIOUS|CMMsg.MASK_ALWAYS|CMMsg.TYP_ACID,Weapon.TYPE_BURNING,L("The acid clumps <DAMAGE> <T-NAME>!"));
