...
public class Prayer_CauseExhaustion extends Prayer
{
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
    {
        ...
        if(success)
        {
            ...
            // what happened.
            FullMsg msg=new FullMsg(mob,target,this,CMMsg.MASK_MALICIOUS|affectType(auto),(auto?"A light fatigue overcomes <T-NAME>.":"^S<S-NAME> "+prayWord(mob)+" for fatigue to overcome <T-NAMESELF>!^?"));
            if(mob.location().okMessage(mob,msg))
            {
                mob.location().send(mob,msg);
                if(msg.value()<=0)
                {
                    int harming=Dice.roll(5,adjustedLevel(mob,asLevel),20);
