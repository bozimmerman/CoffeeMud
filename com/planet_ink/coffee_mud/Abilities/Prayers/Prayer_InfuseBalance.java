	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(target instanceof Room)
		{
			...
			for(final Enumeration e=A.getMetroMap();e.hasMoreElements();)
			{
				...
				{
					mob.tell(_("There is already a balanced place of @x1 in this area at @x2.",D.Name(),R.displayText(mob)));
					return false;
				}
			}
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?_("A holy balanced aura appears around <T-NAME>."):_("^S<S-NAME> @x1 to infuse a holy balanced aura around <T-NAMESELF>.^?",prayForWord(mob)));
			...
			beneficialWordsFizzle(mob,target,_("<S-NAME> @x1 to infuse a holy balanced aura in <T-NAMESELF>, but fail(s).",prayForWord(mob)));
