...
*/
public class Prayer_AnimateShadow extends Prayer
{
	@Override
	public String ID()
	{
		return "Prayer_AnimateShadow";
	}

	private final static String	localizedName	= CMLib.lang().L("Animate Shadow");
	...
	private final static String	localizedDiplayText	= CMLib.lang().L("Newly animated shadow");
	...
	protected MOB makeUndeadFrom(final Room R, final DeadBody body, final Race bodyR, final MOB mob, final int level)
	{
		...
		final String undeadDesc=L("In undeath, it became a dark incorporeal spirit.");
		...
		final Race undeadR = CMLib.utensils().getMixedRace(bodyR.ID(), "Shadow", false);
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			mob.tell(L("This creature is too weak to create a shadow from."));
			return false;
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),
					auto?"":L("^S<S-NAME> @x1 to animate <T-NAMESELF> as a shadow.^?",prayForWord(mob)));
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				final int undeadLevel = getUndeadLevel(mob,9,body.phyStats().level());
