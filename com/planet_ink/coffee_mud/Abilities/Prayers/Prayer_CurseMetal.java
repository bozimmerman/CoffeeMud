...
public class Prayer_CurseMetal extends Prayer
{
	...
	public boolean okAffect(Environmental myHost, Affect msg)
	{
		...
		{
			msg.source().tell(affected.displayName()+" is filled with unholy heat!");
			return false;
		}
		return true;
	}
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		...
		for(int i=0;i<mob.inventorySize();i++)
		{
			   ...
			   &&(!mob.amDead()))
			{
				...
				ExternalPlay.postDamage(invoker,mob,this,damage,Affect.MASK_GENERAL|Affect.TYP_FIRE,Weapon.TYPE_BURSTING,item.displayName()+" <DAMAGE> <T-NAME>!");
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto)
	{
		...
		if(success)
		{
			...
			// what happened.
			invoker=mob;
			FullMsg msg=new FullMsg(mob,target,this,affectType(auto),"^S<S-NAME> "+prayForWord(mob)+" to curse <T-NAMESELF>.^?");
