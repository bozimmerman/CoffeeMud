...
public class Prayer_Demonshield extends Prayer
{
				   ...
				   &&(!CMLib.flags().isEvil(source)))
				{
					...
					if(source.location().okMessage(source,msg2))
					{
						...
						if(msg2.value()<=0)
						{
							...
							CMLib.combat().postDamage(mob,source,this,damage,CMMsg.MASK_GENERAL|CMMsg.TYP_FIRE,Weapon.TYPE_BURNING,"The unholy flames around <S-NAME> flare and <DAMAGE> <T-NAME>!");
						}
					}
				}
			}
	...
	public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			// what happened.
			CMMsg msg=CMClass.getMsg(mob,target,this,affectType(auto),((auto?"":"^S<S-NAME> "+prayWord(mob)+".  ")+"A field of unholy flames erupt(s) around <T-NAME>!^?")+CMProps.msp("fireball.wav",10));
